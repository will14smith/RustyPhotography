...
use crate::{Config, Photograph, Layout};

pub struct Client {
    dynamo: Arc<dyn rusoto_dynamodb::DynamoDb + Send + Sync>,
    config: Config,
}
...
impl Client {
    pub fn new(dynamo: Arc<dyn rusoto_dynamodb::DynamoDb + Send + Sync>, config: Config) -> Client {
        Client {
            dynamo,
            config,
        }
    ...
    pub fn update_photograph(&self, id: uuid::Uuid, updates: HashMap<String, rusoto_dynamodb::AttributeValueUpdate>) -> Result<Photograph, String> {
